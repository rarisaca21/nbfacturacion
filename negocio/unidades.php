	...
	class Unidades{  
        ...
        public function board($post){
            try{
                ...
                $i = (($post['current'] - 1) * 10);
                $f = $i + 10;
                $rows = $this->db->query("SELECT * FROM unidades ".$query. " ORDER BY id DESC LIMIT ".$i.", 10")->fetchAll(PDO::FETCH_ASSOC);
                $total = $this->db->query("SELECT COUNT(*) as total FROM unidades ".$query)->fetchAll(PDO::FETCH_ASSOC);
        ...
        public function searchById($post){
            try{
                $rows = $this->db->query("SELECT * FROM unidades WHERE id = " . $_POST['id'])->fetchAll(PDO::FETCH_ASSOC); 
            } catch (PDOException $e){
                $this->answer['err']['status'] = 1;
                $this->answer['err']['msg'] = $e->getMessage();
                return json_encode($this->answer);
        ...
        public function list($post){
            try{
                $rows = $this->db->query("SELECT * FROM unidades ")->fetchAll(PDO::FETCH_ASSOC); 
            } catch (PDOException $e){
                $this->answer['err']['status'] = 1;
                $this->answer['err']['msg'] = $e->getMessage();
                return json_encode($this->answer);
