	...
	class Comprobantes{  
        ...
        public function board($post){
            try{
                ...
                $i = (($post['current'] - 1) * 10);
                $f = $i + 10;
                $rows = $this->db->query("SELECT * FROM comprobantes ".$query. " ORDER BY id DESC LIMIT ".$i.", 10")->fetchAll(PDO::FETCH_ASSOC);
                $total = $this->db->query("SELECT COUNT(*) as total FROM comprobantes ".$query)->fetchAll(PDO::FETCH_ASSOC);
        ...
        public function update($post){
            try{
                $data = $this->db->update("comprobantes", [
                     "nombre" => $post['nombre'],
                    "codigo" => $post['codigo'],
                    "estado" => $post['estado']
                ], [
        ...
        public function searchById($post){
            try{
                $rows = $this->db->query("SELECT * FROM comprobantes WHERE id = " . $_POST['id'])->fetchAll(PDO::FETCH_ASSOC); 
            } catch (PDOException $e){
                $this->answer['err']['status'] = 1;
                $this->answer['err']['msg'] = $e->getMessage();
                return json_encode($this->answer);
        ...
        public function deleteById($post){
            try{
                $res = $this->db->delete("comprobantes", [
                    "id[=]" => $_POST['id']
                ]); 
                $rows = $res->rowCount();
            } catch (PDOException $e){
